...
def TrainSupportVectorMachine():
    df = pd.read_csv('F:/Appliances_All_20180717.csv', header=None)
    df.tail()
    # select setosa and versicolor
    y = df.iloc[:, 0].values#數據開始行:數據結束行
    # extract sepal length and petal length
    X = df.iloc[:, [2, 4]].values#數據開始行:數據結束行,特徵1,特徵2
    # plot data
    plt.scatter(X[:3999, 0], X[:3999, 1],color='red', marker='o', label='Ten Watt')
    plt.scatter(X[4000:7999, 0], X[4000:7999, 1],color='blue', marker='x', label='Twenty Watt')
    plt.scatter(X[8000:9999, 0], X[8000:9999, 1],color='green', marker='^', label='JeinPoLi Energy Saving Lamps')
...
def SupportVectorMachinePrediction(svm):
    ...
    db = MySQLdb.connect(host="timmy61109.ddns.net", user="root", passwd="openele", db="power", charset="utf8")
